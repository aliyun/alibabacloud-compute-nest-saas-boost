...
public class LocalCertStorageHelper {
    ...
    public BaseResult<Boolean> putCert(String certName, String certContent) {
        ...
        try {
            ...
            try (FileWriter writer = new FileWriter(file, false)) {
                ...
                return BaseResult.fail("Error when writing the certificate: " + e.getMessage());
            }
    ...
    public BaseResult<Boolean> deleteCert(String certName) {
        ...
        File file = new File(filePath);
        if (!file.exists()) {
            return BaseResult.fail("Certificate does not exist");
        } else {
            try {
                Files.delete(Paths.get(filePath));
            } catch (Exception e) {
                return BaseResult.fail("Error when deleting the certificate: " + e.getMessage());
            }
            return BaseResult.success(true);
        }
    }
