...
public class BaseWechatPayClientImpl implements BaseWechatPayClient {
    ...
    public String nativePay(OrderDTO order) {
        try {
            ...
            IJPayHttpResponse response = WxPayApi.v3(RequestMethodEnum.POST,
                    wechatPayConfig.getGateway(),
                    BasePayApiEnum.NATIVE_PAY.toString(),
                    wechatPayConfig.getMchId(),
                    wechatPayConfig.getMchSerialNo(),
                    null, privateKey, JSONUtil.toJsonStr(unifiedOrderModel), nonceStr, timestamp,
                    AuthTypeEnum.RSA.getCode(), null);
    ...
    public String queryOutTrade(String outTradeNo) {
        try {
            ...
            IJPayHttpResponse response = WxPayApi.v3(RequestMethodEnum.POST,
                    ...
                    wechatPayConfig.getMchId(),
                    wechatPayConfig.getMchSerialNo(),
                    null, privateKey, JSONUtil.toJsonStr(params), nonceStr, timestamp,
                    AuthTypeEnum.RSA.getCode(), null);
    ...
    private void createClientWithCert() {
        try {
            ...
            String wechatCertPath = String.format("%s/%s", PayChannel.WECHATPAY.getDisplayName(),
                    wechatPayConfig.getPrivateKeyPath());
            BaseResult<String> wechatPay = ossClient.getObjectContent(bucketName, wechatCertPath);
            if (StringUtils.isNotEmpty(wechatPay.getData())) {
    ...
    private PrivateKey getPrivateKey() {
        ...
        BaseResult<String> wechatPay = ossClient.getObjectContent(bucketName, wechatCertPath);
        if (StringUtils.isNotEmpty(wechatPay.getData())) {
            return PemUtil.loadPrivateKey(wechatPay.getData());
        }
        return null;
    }
}
